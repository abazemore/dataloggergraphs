...
server <- function(input, output) {
  ...
  envdata <- eventReactive(input$submit, {
    ...
    if (length(input$files) < 0) {
      message('Using uploaded data')
      parse_brand(input$files, brand = input$brand, site = input$site)
    }
  })
  ...
  #     datalist <- c('../data/previous')
  #   }
  #   else if (nrow(input$files) > 0) {
  #     message('Loadig uploaded data')
  #     message(input$files$datapath)
  #   }
  #   else {
 ...
 #    envdata <- parse_datalogger(datalist(),
 #           site = input$site,
 #           brand = input$brand)
 #  })
 #
 ...
 #      'graph_type',
 #      'Graph type:',
 #      choices = c('All data' = 'all', 'Monthly summary' = 'summary')
 #    )
 #  })
 #  # Sidebar date range
