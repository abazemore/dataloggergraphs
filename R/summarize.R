...
#' @param percentile Boolean to include 1-99 percentile columns to trim outliers
#'
...
#' @examples
#' \dontrun{
#' dplyr::summarize_site(envdata, start_date = "2024-01-01", type = "annual")
#' }
summarize_site <- function(envdata,
  ...
  if (!all(is.na(subset$temp))) {
    trh_summary <- dplyr::filter(subset, dplyr::when_any(!is.na(temp), !is.na(RH))) |>
      dplyr::summarize(
        ...
        p01_temp = round(stats::quantile(temp, 0.01, na.rm = TRUE), 1),
        p99_temp = round(stats::quantile(temp, 0.99, na.rm = TRUE), 1),
        ...
        p01_RH = round(stats::quantile(RH, 0.01, na.rm = TRUE), 1),
        p99_RH = round(stats::quantile(RH, 0.99, na.rm = TRUE), 1),
  ...
  if (!all(is.na(subset$lux))) {
    light_summary <- dplyr::filter(subset, !is.na(lux)) |>
      ...
      p01_lux = round(stats::quantile(lux, 0.01, na.rm = TRUE), 1),
      p99_lux = round(stats::quantile(lux, 0.99, na.rm = TRUE), 1),
...
#' @examples
#' \dontrun{
#' set_minmax(standard = "PAS 198", max_temp = 25)
#' }
...
#' @examples
#' \dontrun{
#'  compliance(envdata, standard = "Icon")
#' }
compliance <- function(envdata,
  ...
  rated <-  dplyr::group_by(subset, site, location) |>
    ...
    tidyr::pivot_longer(
      cols = tidyr::starts_with(c("temp", "RH", "standard")),
      names_to = "rating",
      values_to = "value",
      values_drop_na = TRUE
    ) |>
